# This code use three files: Background_genes, Factor_Label (including 269 DEGs), and go_annotations

library("goseq")
library("GO.db")
library("qvalue")
# capture list of genes for functional enrichment testing
factor_labeling = read.table("Factor_Label.txt", row.names=2, header=F)
...
background = read.table("Background_genes.txt", header=T, row.names=1)
